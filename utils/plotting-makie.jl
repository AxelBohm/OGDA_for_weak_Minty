...
function create_performance_plots(data, VI::AbstractVI, problem_params, algorithms)
    ...
    if VI.dim < 3
        plot_iterates(data, VI, path)
    end

    if occursin("ratio-game", path)
        plot_heatmap(VI, path)
        plot_iterates_for_ratio_game(data, VI, path)
    end
...
function plot_iterates_for_ratio_game(data, VI::AbstractVI, path, num_mark=10)
    """Plot trajectory of iterates (only possible if problem is 2-d)"""
    ...
    n = length(data)
    for (k, alg) in enumerate(data)
        mat = reshape(alg.iterates, 4, :)
        x = convert(Vector{Float64}, mat[1, :])
        y = convert(Vector{Float64}, mat[3, :])
